defmodule GenRouter do
  ...
  """
  @spec match_in_scope(module(), atom(), [route], [atom()], String.t(), Conn.t(), Keyword.t()) ::
          Conn.t()
  def match_in_scope(
        router_module,
        scope,
    ...
    Enum.reduce(routes, :next_scope, fn {path, controller, action}, acc ->
      if acc === :next_scope do
        ...
        if result === true or (is_map(result) and not Map.equal?(result, %{})) do
          conn =
            if is_map(result), do: %{conn | params: Map.merge(conn.params, result)}, else: conn
  ...
  defmacro scope(scope, scope_path, do: block) do
    ...
    skip = Map.put(%{}, scope, false) |> Macro.escape()
